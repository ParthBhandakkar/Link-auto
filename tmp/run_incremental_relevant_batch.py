...
async def main() -> None:
    ...
    try:
        ...
        print(f"Existing sheet jobs: {len(existing_links)}")
        print("Scraping fresh jobs without clearing the sheet...")

        all_scraped_jobs = []
        selected_jobs = []
        seen_urls: set[str] = set(existing_links)
        ...
        for keyword in PROFILE.get("job_search_keywords", []):
            ...
            for job in filtered_jobs:
                ...
                try:
                    ...
                    if not opened:
                        continue
                    if await orch.applier._is_already_applied():
                        seen_urls.add(job.url)
                        continue
                    if await orch.applier._find_easy_apply_button():
                        selected_jobs.append(job)
                        seen_urls.add(job.url)
                        print(f"Selected: {job.title} | {job.company}")
                except Exception:
                    continue
        ...
        if not selected_jobs:
            print("No new relevant Easy Apply jobs available")
            return
        ...
        orch.stats.session_end = datetime.now()
